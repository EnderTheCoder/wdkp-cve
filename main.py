...
from util import sql_time_to_timestamp, timestamp_to_sql_time
    ...
    while True:
        ...
        if option == 2:
            ...
            if input("\t是否确认删除[y/N]") == 'y':
                ...
                DeleteLocationRequest(user_id, run_records[run_data_id]['qssj'],
                                      run_records[run_data_id]['jssj']).send()
                print('删除成功')
            else:
                print('删除取消')
            pass
        ...
        if option == 7:
            if len(run_records) == 0:
                print("警告：不存在已有的跑步记录，无法自动设定参数，操作终止")
                continue
            if run_records[-1]['qssj'].split(' ')[0] == timestamp_to_sql_time(int(time.time())).split(' ')[0]:
                print("警告：存在和当前日期相同的跑步记录，终止操作")
                continue
            target_time_full = timestamp_to_sql_time(int(time.time())).split(' ')[
                                   0] + f' 08:{random.randrange(0, 5)}:{random.randrange(0, 59)}'
            print(f"使用随机时间：{target_time_full}")
            last_record = run_records[-1]
            phone_serial = last_record['phoneno']
            app_version = last_record['version']
            phones = []
            phone_table = PrettyTable(['编号', '手机号码'])
            i = 0
            for dir_name in os.listdir("data/"):
            ...
            with open(f"data/{target_phone}/run_section/{target_record['id']}.json") as f:
                sections = list(json.load(f))
                for s_idx, section in tqdm(enumerate(sections), '写入区段数据'):
                    if s_idx == len(sections) - 1:
                        section['rtime'] = util.offset_time(section['rtime'], random.randrange(0, 60 * 5))
            ...
            with open(f"data/{target_phone}/run_location/{target_record['id']}.json") as f:
                locations = list(json.load(f))
                for l_idx, location in tqdm(enumerate(locations), '写入位置关键点'):
                    fail_cnt = 0
                    try:
                        InsertLocationRequest(dict(location), user_id, timestamp_offset).send()
                    except UnparsableRequestException:
